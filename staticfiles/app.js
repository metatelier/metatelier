...
let busquedaActual = '';
let bolsaCompras = JSON.parse(localStorage.getItem('met_cart') || '[]');
let adminPIN = sessionStorage.getItem('met_admin_pin') || '';
let editandoId = null;
...
function guardarBolsa() {
    localStorage.setItem('met_cart', JSON.stringify(bolsaCompras));
}
...
if (btnLogoutAdmin) {
    btnLogoutAdmin.addEventListener('click', () => {
        adminPIN = '';
        sessionStorage.removeItem('met_admin_pin');
        sessionStorage.removeItem('met_admin_unlocked');
...
async function solicitarAutenticacionAdmin() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            sessionStorage.setItem('met_admin_pin', adminPIN);
            sessionStorage.setItem('met_admin_unlocked', 'true');
    ...
    btnExportBackup.addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                a.href = url;
                a.download = `met_atelier_respaldo_${fechaHoy}.json`;
